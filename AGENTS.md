...
- Сомнительная voice-запись получает Telegram inline-кнопки `✅ Подтвердить`, `❌ Отмена` и `Открыть отчёт`.
- Callback data короткие и стабильные: `confirm:<record_id>` и `cancel:<record_id>`.
- Callback должен быть идемпотентным: повторное нажатие не ломает данные и не меняет уже принятое решение.
- WebApp имеет вкладку «Проверка»: пользователь в рамках server-derived `shop_id` может отредактировать товары и подтвердить или отменить review-запись. Редактирование товара сохраняет поля, но review-запись входит в выручку только после явного confirm.
