...
- Сомнительная запись должна оставаться `needs_review` до Telegram decision.
...
- WebApp - панель управления и просмотра, не место подтверждения сомнительных voice-записей.
- Нижняя навигация: `Отчёт`, `Записи`, `Продавцы`.
- Не возвращать пользовательскую вкладку `/review` с confirm/cancel.
- `needs_review` в WebApp показывать как `Нужно подтвердить в Telegram`.
...
- `needs_review` - запись сохранена, но не входит в выручку до Telegram confirm.
...
- Не использовать формулировки про учебный проект, куратора или демо ради проверки.
