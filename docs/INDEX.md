...
- [Acceptance matrix](./features/acceptance-matrix.md)
...
- Сомнительные voice-записи подтверждаются или отменяются Telegram inline-кнопками либо WebApp вкладкой `Проверка`.
- Bot success возможен только после подтверждённой записи sale и sale_items.
- Спорные позиции идут в «Нужно проверить» и не входят в выручку, пока сама позиция не станет `processed`; processed sibling items могут учитываться сразу.
- Исключение товара выполняется через soft delete.
- Карточки товаров редактируются inline; review edit сохраняет поля, а confirm выполняется отдельным review action.
