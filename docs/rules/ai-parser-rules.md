...
- менять prompt, схему, очистку или детерминированную проверку только вместе;
- добавлять регрессионный тест для исправляемого примера и пограничного случая;
- не снижать порог и не разрешать догадки ради «красивого» результата;
- сохранять исходные `raw_text` и `parser_json` для диагностики.
