...
Confirm переводит sale/voice в `processed`. Cancel переводит sale/voice в `cancelled` и soft-delete active items. Решение может прийти из Telegram callback или WebApp `/review`; обычный WebApp item edit/delete сам по себе не подтверждает и не отменяет review voice-записи.
