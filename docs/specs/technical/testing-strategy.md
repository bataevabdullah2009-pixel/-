...
- Telegram review keyboard из `✅ Подтвердить`, `❌ Отмена`, `Открыть отчёт`;
...
- WebApp review page confirm/cancel и cross-shop callback denial.
...
Ручной WebApp smoke 25 июня 2026 проверил update quantity, update price, reload persistence, soft delete, reload после delete и restore. После проверки исходные значения позиции восстановлены. 30 июня 2026 локальные tests покрыли короткий callback contract, confirm/cancel, cancelled report filtering, cross-shop denial и новый WebApp review contract; реальный Telegram smoke остаётся backlog после deploy.
