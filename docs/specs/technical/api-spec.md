...
- `POST /api/telegram/webhook` — Telegram updates. Проверяет `x-telegram-bot-api-secret-token`, затем вызывает общий bot update processor. Voice updates сохраняют продажи; callback updates обрабатывают `confirm:<record_id>` / `cancel:<record_id>` для review sales.
