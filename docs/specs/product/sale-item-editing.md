...
Review-запись начинает входить в выручку только после Telegram `✅ Подтвердить`.
...
Чтобы review-запись вошла в выручку, нужно нажать `✅ Подтвердить` в Telegram.

Это правило защищает продукт от скрытого подтверждения через WebApp.
...
Если previous status был `needs_review`, позиция снова требует Telegram confirm.
...
6. Review item не входит в report total до Telegram confirm.
...
3. Изменение даты продажи.
4. Изменение продавца продажи.
5. Редактирование audio/STT/parser данных.
6. Физическое удаление row.
