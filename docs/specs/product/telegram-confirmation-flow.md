...
1. Telegram должен быть единственным местом подтверждения сомнительных voice-записей.
...
4. `WebApp` показывает результат после обновления, но не решает review.
...
10. Для каждого active item проверяются product, quantity, price, total.
11. Если нет валидных items, confirm не выполняется.
12. Если не все active items валидны, confirm не выполняется.
13. Валидные items получают `status = processed`.
14. Items получают `confidence = 1`.
15. Items получают пересчитанный `total`.
16. Sale получает `status = processed`.
17. Voice record получает `status = processed`.
18. Sale total становится суммой confirmable items.
19. Bot редактирует или отправляет сообщение:
...
2. WebApp не показывает confirm/cancel controls.
3. Needs_review record показывает `Нужно подтвердить в Telegram`.
4. Confirmed sale появляется в active revenue.
5. Cancelled sale не появляется в active revenue.
6. Audio and transcript остаются в records.
...
5. Incomplete items get message asking for product, quantity and price.
6. Supabase errors are logged with callback metadata.
7. Telegram edit failure falls back to `ctx.reply`.
...
10. WebApp shows review records as Telegram-confirm-needed.

## 19. Out of scope

1. WebApp confirm/cancel.
2. Per-item Telegram confirmation.
3. Multi-step correction conversation in Telegram.
4. Replacing STT/parser.
5. Physical deletion of cancelled sale items.
