...
WebApp показывает review-состояние и имеет отдельную вкладку «Проверка» для owner-level confirm/cancel в рамках server-derived магазина.
...
1. «Диагностика Telegram».
2. «Подтвердить позицию».
3. Любую URL-кнопку.

Кнопка отчёта остаётся в `/start`, reply keyboard и menu button.

В review-message `Открыть отчёт` создаётся только как Telegram `web_app` button.
...
Legacy `voice_sale_review:<action>:<sale_id>` принимается для уже отправленных сообщений, но новые сообщения используют только короткий формат.
...
2. Все не исключённые active позиции должны быть валидны.
...
Если WebApp открыт во время Telegram callback, пользователь обновляет экран или нажимает refresh. Вкладка «Проверка» после своих server actions revalidate отчёт, записи, проверку и продавцов.
...
2. Сомнительная запись получает confirm, cancel и `Открыть отчёт`.
3. В review-message `Открыть отчёт` является `web_app` кнопкой.
...
11. WebApp показывает confirm/cancel controls только во вкладке «Проверка».
12. Tests покрывают confirm, cancel, keyboard contract и cross-shop denial.

## Не входит в scope

1. Подтверждение отдельных позиций в WebApp.
2. Физическое удаление sale.
3. Изменение STT.
4. Изменение parser prompt.
5. Клиентский выбор магазина.
