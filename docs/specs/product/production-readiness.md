...
Реализовано: inline и menu `web_app` buttons, проверка канонического Web App URL, официальный Telegram SDK, ожидание WebApp object, `ready()`/`expand()`, общий initData API helper, точные auth error codes, owner/seller/shop lookup, server-derived shop isolation, webhook secret, server-only service role, RLS без анонимного чтения, RPC persistence с rollout fallback, soft delete, stage logging, безопасные сообщения UI и regression tests.
