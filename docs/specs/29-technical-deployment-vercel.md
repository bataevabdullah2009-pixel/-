...
До проверки schema нужно проверить operational status самого проекта. `INACTIVE`/paused project делает недоступными seller lookup, сохранение voice и WebApp server queries, хотя Vercel deployment может оставаться `READY`.
...
Дополнительные external smoke-команды:
...
`smoke:production` является write-smoke: он требует явного `PRODUCTION_SMOKE_CONFIRM=voice-sales-log`, создаёт только временные записи с уникальным префиксом и обязан выполнить адресную cleanup-проверку.
...
Автоматический `smoke:webapp` проверяет signed initData, session cookie, четыре страницы, Next assets и Telegram SDK, но не заменяет ручной запуск в Telegram Android/Desktop/Web: только клиент Telegram может подтвердить фактический `window.Telegram.WebApp` и переданный launch initData.
...
Если одновременно перестали работать voice и WebApp, сначала проверить Supabase project status и DNS/API availability. Падение voice на `seller_resolve` означает, что Telegram download, STT и parser ещё не запускались.
