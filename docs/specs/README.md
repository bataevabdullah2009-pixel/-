...
Главные правила: raw Telegram initData проверяется через bot token, fallback auth включается явно, `shop_id` не принимается от клиента, report читает sale_items только через sales текущего shop, уверенные voice-продажи сразу входят в отчёт, спорные voice-записи решаются через Telegram `✅ Подтвердить` / `❌ Отмена` или WebApp вкладку «Проверка».
