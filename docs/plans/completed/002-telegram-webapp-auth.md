...
Проверено локально: 55 tests, lint без ошибок, production build успешен. Исторический server smoke подтвердил вызовы `web_app_ready`/`web_app_expand`, ответы missing/invalid `401` с правильными кодами, успешный `200` auth существующего active seller, выдачу cookie и загрузку `/daily-report` без auth error. Production webhook был готов, без pending/error и с allowed update `message`; конкретный устаревший URL удалён из актуальной документации.
