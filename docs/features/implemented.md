...
- Мягкое удаление позиции через `deleted_at`, `excluded_by_owner` и статус `excluded` с возможностью восстановления.

## Веб-панель

- Страницы `/daily-report`, `/records` и `/sellers`.
- Фильтры даты и продавца, поиск по тексту, карточки и пустые состояния.
- Правка названия, положительных количества и цены любой активной позиции с серверным пересчётом и безопасными ошибками.
...
- Исключение статусов `needs_price`, `needs_review`, `failed`, `excluded` и мягко удалённых позиций.
