...
Вкладка `Проверка` показывает active `needs_review` позиции отдельными компактными карточками. Подтверждение, отмена и `Подтвердить всё` выполняются через server actions и синхронизируются с теми же статусами, что Telegram inline-кнопки.
