...
Сомнительная запись получает `needs_review`, не входит в выручку до решения и подтверждается Telegram inline-кнопками `✅ Подтвердить` / `❌ Отмена` либо вкладкой WebApp `Проверка`. В сообщении сомнительной записи нет кнопки `Открыть отчёт`.
...
Сохранение вручную реально обновляет строку `sale_items`, пересчитывает `total`, ставит item в `processed` и убирает ошибку цены. Если parent sale ещё `needs_review`, выручка не появляется до явного Telegram/WebApp confirm, потому что отчёт считает только parent sale `processed`.
...
1. `Сникерс, 3 штуки по 200 рублей. Буханка хлеба, 5 штук по 50 рублей.` создаёт две `sale_items` и сумму `850 ₽`.
2. `Сникерс, 5 штук по 100 рублей` сохраняется как `processed`, входит в выручку и виден в WebApp.
3. Длинная сомнительная запись сохраняется как `needs_review`, не входит в выручку и получает только `✅ Подтвердить` / `❌ Отмена`.
4. `✅ Подтвердить` переводит запись в `processed`, добавляет товары в выручку и обновляет WebApp после refresh.
5. `❌ Отмена` переводит запись в `cancelled`, товары не входят в выручку.
