...
Confirm работает на уровне `sale_items`: если в корзине есть хотя бы одна полная позиция, она переводится в `processed`, попадает в `sales.total_amount` и начинает входить в выручку. Неполные позиции той же записи остаются active `needs_review` и продолжают отображаться во вкладке `Проверка`. Если нет ни одной полной позиции, confirm не выполняется и пользователь видит сообщение `Не удалось подтвердить: нет ни одной полной позиции.`
...
Карандаш открывает compact edit mode с полями `Товар`, `Количество`, `Единица`, `Цена, ₽`, кнопками `Сохранить` и `Отмена`. Единица поддерживает `шт`, `кг` и `г`; для граммов сумма считается как доля килограмма от цены за кг.
